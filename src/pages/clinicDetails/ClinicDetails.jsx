...
const ClinicDetails = () => {
  ...
  useEffect(() => {
     apiWrapper({ url: `${process.env.REACT_APP_PATIENT_SERVER_URL_FAKE}/clinic/details/${clinicId.clinicId}`, method: 'POST' }).then(
      (res) => {
        setData(res);
        setLoading(false);
      },
  ...
  return (
    <div>
      <>
        ...
        { isModalShow ?     
        (
            <BookingModal data={data} show={isModalShow} modalClosed = {closeModal}/>
        ) : null
      }
      </>
    </div>
